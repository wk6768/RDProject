...
{
    public sealed class 二厂生产部负责人 : NativeActivity
    {
        public OutArgument<bool> Answer_二厂生产部负责人 { get; set; }
        public OutArgument<string> BookMarkName { get; set; }

        protected override void Execute(NativeActivityContext context)
        {
            Debug.WriteLine("流入:二厂生产部负责人");
            Bookmark bookmark = context.CreateBookmark("二厂生产部负责人", DoSome);
        }

        private void DoSome(NativeActivityContext context, Bookmark bookmark, object value)
        {
            Debug.WriteLine("执行:二厂生产部负责人");
            ...
            context.SetValue(Answer_二厂生产部负责人, Convert.ToBoolean(b));
